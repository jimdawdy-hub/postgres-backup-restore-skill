...
If the application keeps multiple logical environments or data generations in
one cluster, add a project-specific identity check to the pre-flight. Derive the
expected value from the authorized operation or reviewed service manifest, not
from whichever generation currently has the largest ID:

```bash
expected_generation_id='<authorized generation>'
marker_count="$(psql -h /path/to/socket -p 55432 -d mydb -v ON_ERROR_STOP=1 \
  -v expected_generation_id="$expected_generation_id" -At <<'SQL'
SELECT count(*)
FROM application_current_generation
WHERE generation_id = :'expected_generation_id'::bigint;
SQL
)" || exit 1
if [[ "$marker_count" != 1 ]]; then
  printf 'identity check failed: expected one marker, found %s\n' \
    "$marker_count" >&2
  exit 1
fi
```

The table and column names are application-specific; do not copy those
placeholders blindly. The invariant is that the expected identity is explicit,
the live database supplies independent evidence, and absence, mismatch, or
ambiguity stops the backup. Include that logical environment or generation in
the backup receipt and, where useful, in the artifact name.
...
When several environments or generations coexist, include the verified logical
identity as well, such as `mydb-generation-7-2026-08-04-pre-migration.pgdir`.
